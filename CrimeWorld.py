...
# center grid with 32 x 32 grid on given x,y coord (dim 0 = y)
def centeredGrid(A,x,y):
    sz = A.shape #A = total crimes
    c = int(sz[0]/2) #single shape of the matrix
    dx = int(np.floor(sz[1]/2 - x))     # floor but returns int type
    dy = int(np.floor(sz[0]/2 - y))
    pr = int(32/2) # pr to be the matrix grid lock
    CC = np.roll(A,(dy,dx),axis=(0,1))
    res = CC[c-pr:c+pr, c-pr:c+pr]
    return res
...
class CrimeWorld():
    ...
    def set_params(self,x,y):
        ...
        self.P_M = 32                   # police centered grid
        self.dt = 4.0/100.0             # time step
        
        # burglar parameters
        self.w = 1.0/15.0               # repeat victimization time scale
        self.gamma = 0.019              # burglar spawn rate
        self.eta = 0.05                 # neighbor influence (diffusion rate)
    ...
    def new_episode(self):
        ...
        # running sum of crime, with a window size
    ...
    # returns the sum of B and n matrices
    def result_matrixsum(self, a, R):
        a_temp = centeredGrid(self.B, self.policeX, self.policeY)
        new_B = middleMatrix(a_temp, 5).sum()
        a_temp = centeredGrid(self.n, self.policeX, self.policeY)
        new_n = middleMatrix(a_temp, 5).sum()
        return new_B, new_n
    ...
    def reset(self):
        ...
        # placing police back to original position
        self.policeX = self.policeX0
        self.policeY = self.policeY0
        self.P[self.policeY, self.policeX] = 1
        
        # if self.P[self.policeY, self.policeX] < 1:
        #     self.P[self.policeY, self.policeX] = 1
        return self.get_state()
        
    # saving checkpoint for the use in trianing the dqn model
    def save_checkpoint(self, file):
        np.savez(file, B=self.B, C=self.C, n=self.n, P=self.P, D=self.D, totalC=self.totalC, C_buf=self.C_buf)
    ...
    # use for making a step in dqn model to predict the next reward given an action  
    def step(self,a):
    ...
    def make_action(self,a):
        ...
        # r = 5
        # return -deltaC[self.policeY-r:self.policeY+r, self.policeX-r:self.policeX+r].sum() -1
        # reward = -middleMatrix(deltaC,r).sum() #max(0,-middleMatrix(deltaC,r).sum())-0.1
        
        # return only local position values from the region around agent
        reward = self.B[self.policeY, self.policeX]
        return reward
