...
#         with open('result_output_new.csv', mode='a') as csv_file:
#             writer = csv.writer(csv_file, delimiter=',')
#             writer.writerows([[self.policeX, self.policeY, a, R, new_B, new_n]])
        return new_B, new_n
    
    def save_result_file(self, a, R):
        a_temp = centeredGrid(self.B, self.policeX, self.policeY)
        new_B = middleMatrix(a_temp, 5).sum()
        a_temp = centeredGrid(self.n, self.policeX, self.policeY)
        new_n = middleMatrix(a_temp, 5).sum()
...
#         return centeredGrid(self.C,self.policeX,self.policeY)
    ...
    def make_action(self,a):
        ...
        self.P[self.policeY,self.policeX] -= 1
#         print("remove police from ", self.policeY, self.policeX)
...
#         else:
        ...
        # add in new location
        self.P[self.policeY,self.policeX] += 1
#         print("new police action update", self.policeY, self.policeX) 
...
#         reward = -middleMatrix(deltaC,r).sum() #max(0,-middleMatrix(deltaC,r).sum())-0.1
        reward = self.B[self.policeY, self.policeX] #np.maximum(0, -middleMatrix(self.B,r)).sum()-0.01 
        return reward
